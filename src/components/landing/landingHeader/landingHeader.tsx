...
import { LandingHeaderProps } from "components/landing/landingContent";
    ...
    return (
        <Container sx={styles.header} component="header">
            ...
            <Box sx={styles.headerCtaLogin}>
                <Typography variant="h6" sx={{ fontWeight: 700, fontSize: 14, mb: 1 }}>{ctaLogin}</Typography>
                <Typography variant="body2" sx={{ fontSize: 14, mb: 1 }}>{ctaLoginSub}</Typography>
                <Button variant="outlined" color="primary" to={ROUTES.LOGIN} component={Link} sx={{ fontWeight: 700 }}>
                    {ctaLoginButton}<ArrowLeft/>
                </Button>
            </Box>
        </Container>
    );
