...
function ShiftSchedule() {
  ...
  const onShiftClick = useCallback(
    (args) => {
      ...
      const position = myPositions.find((p) => {
        return p.id === shift.position;
      });
      //drop down with all existing guards
      setHeader("<div>שינוי שומר  " + shift.guardName + " בעמדה " + position.name + '</div><div class="employee-shifts-day">' + formatDate("HH:mm", new Date(shift.start)) + " עד " + formatDate("HH:mm", new Date(shift.end)) + "</div>");
      setEdit(true);
      setTempShift({ ...shift });
      setGuardName(shift.guardName);
  ...
  const onShiftCreated = useCallback(
    (args) => {
      const shift = args.event;
      const position = myPositions.find((p) => {
        return p.id === shift.position;
      ...
      //drop down with all existing guards
      setHeader("<div>משמרת חדשה בעמדה " + position.name + '</div><div class="employee-shifts-day">' + formatDate("HH:mm", new Date(shift.start)) + " עד " + formatDate("HH:mm", new Date(shift.end)) + "</div>");
      setEdit(false);
      setTempShift(shift);
      // fill popup form with shift data
      loadPopupForm(shift);
  ...
  const onShiftDeleted = useCallback(
    (args) => {
  ...
  return (
    <div>
      <Eventcalendar className="md-timetable" view={view} data={shifts} resources={myPositions} extendDefaultEvent={myDefaultShift} renderScheduleEventContent={myCustomShift} clickToCreate={true} dragToCreat={true} dragToMove={true} dragToResize={true} dragTimeStep={15} eventDelete={true} onEventClick={onShiftClick} onEventCreated={onShiftCreated} onEventDeleted={onShiftDeleted} />
      <Popup display="bottom" fullScreen={true} contentPadding={false} headerText={headerText} buttons={popupButtons} isOpen={isOpen} onClose={onClose} responsive={responsivePopup} cssClass="employee-shifts-popup">
        <div className="mbsc-form-group">
          <Select value={guardName} data={guards} onChange={guardNameChange} label="בחירת שומר" />
        </div>
        {isEdit && (
